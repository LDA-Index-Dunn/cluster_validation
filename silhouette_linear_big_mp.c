...
#define DATASET "/home/wellington/luna_files/luna_k11_f20_11000000.dat"
#define NUM_CLUSTERS 11
#define NUM_FEATURES 20
#define MAX_INSTANCES 11000000

// #define DATASET "/home/roussian/cluster_validation/datasets/Effectiveness/Digits/digits_k6_f64_1797s.dat"
...
void calculate_centroids(Instance *instances, int num_instances,
                         Centroid *centroids, int * cluster_sizes ) {
    memset(centroids, 0, NUM_CLUSTERS * sizeof(Centroid));
        int *init_position = (int *)malloc(sizeof(int) * NUM_CLUSTERS);
    ...
    {
        //printf("*******%d-Centroid********\n", cluster);
        for (int instance = init_position[cluster]; instance < init_position[cluster] + cluster_sizes[cluster]; instance++)
        {
            //printf("*******%d-Instance********\n", instance_pos);
            for (int j = 0; j < NUM_FEATURES; j++)
            {
                //printf("%d, A - centroid valor %.2f\n", j, centroids[cluster].features[j]);
                //printf("instances %.2f\n",instances[instance_pos].features[j]);
                centroids[cluster].features[j] += instances[instance].features[j];                  

                //printf("%d, D - centroid valor %.2f\n", j, centroids[cluster].features[j]);             
            }
        }
    }
